...
func TestNewServerWiresDemoStore(t *testing.T) {
	...
	if s.store.URL() != demostore.DefaultURL {
		t.Errorf("demo store URL = %q, want %q", s.store.URL(), demostore.DefaultURL)
	}
}
...
func TestDemoStoreStatus(t *testing.T) {
	...
	if d["url"] != demostore.DefaultURL {
		t.Errorf("status url = %v, want %q", d["url"], demostore.DefaultURL)
	}
}
